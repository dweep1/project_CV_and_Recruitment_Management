...
function() 
{
    $('#InteviewPanelSaveBtn').click(function(){
		var from = document.getElementsByName("schedule[0][from]")[0].value;  
		var to = document.getElementsByName("schedule[0][to]")[0].value;
		...
		if(($('#interviewName').val())==""){
			...
			swal("Oops...","Please enter a valid value for interview duration per candidate","error");
		}else if(($('#tableCommon :input').val())==""){
			swal("Oops...","Please enter interview schedule","error");
		}else if(from=="" || to==""){
			swal("Oops...","Please enter interview schedule","error");
		}else if(from < "06:00" || from > "18:00"){
			swal("Oops...","Please enter valid time for interview schedule (from slot)","error");
		}else if(to < "06:00" || to > "18:00"){
			swal("Oops...","Please enter valid time for interview schedule (to slot)","error");
		}else if(from >= to){
			swal("Oops...","Please enter valid time duration for interview schedule","error");
		}else{			
  ...
  <strong class="headingsInADiv"> Convenient Time Slots</strong><br/><hr/>
 	<table width="99%" border="1" cellpadding="5" class="sortable" id="tableCommon">
	  <tbody>
		...
		<tr>
		<td><input id="d[0]" type="date" min="<?php echo (date('Y-m-d'));?>" name="schedule[0][date]" required/></td>
